...
function renderUI(d) {
    ...
    if (tg.initDataUnsafe && tg.initDataUnsafe.user && tg.initDataUnsafe.user.photo_url) {
        ...
        picElem.classList.remove('hidden');
    ...
    if (typeElem && expElem) {
        if(d.is_admin) {
            typeElem.innerHTML = '<i class="fas fa-user-shield text-rose-500 mr-2"></i> Administrator';
            typeElem.className = "text-lg font-bold text-rose-500";
            expElem.innerHTML = '<i class="fas fa-bolt text-yellow-400"></i> God Mode';
            if(btnAdm) btnAdm.classList.remove('hidden');
            if(fbContainer) fbContainer.classList.add('hidden'); // Admin hides form
            if(btnExt) btnExt.classList.add('hidden');
        } else if(d.is_vip) {
            typeElem.innerHTML = '<i class="fas fa-crown text-purple-400 mr-2"></i> VIP';
            typeElem.className = "text-lg font-bold text-purple-400";
            expElem.innerHTML = '<i class="fas fa-infinity"></i> Lifetime';
            if(btnAdm) btnAdm.classList.add('hidden');
            if(fbContainer) fbContainer.classList.add('hidden'); // VIP hides form
            if(btnExt) btnExt.classList.add('hidden');
        } else if(d.is_prem) {
            typeElem.innerHTML = '<i class="fas fa-star text-amber-400 mr-2"></i> Pro';
            typeElem.className = "text-lg font-bold text-amber-400";
            expElem.innerHTML = '<i class="fas fa-clock"></i> Exp: ' + (d.expiry_date || "Unknown");
            if(btnAdm) btnAdm.classList.add('hidden');
            if(fbContainer) fbContainer.classList.remove('hidden'); // Premium shows form
            if(btnExt) btnExt.classList.remove('hidden');
        } else {
            typeElem.innerHTML = '<i class="fas fa-user text-emerald-400 mr-2"></i> Free Plan';
            typeElem.className = "text-lg font-bold text-emerald-400";
            expElem.innerHTML = '<i class="fas fa-circle-check"></i> Basic';
            if(btnAdm) btnAdm.classList.add('hidden');
            if(fbContainer) fbContainer.classList.remove('hidden'); // Free shows form
            if(btnExt) btnExt.classList.add('hidden');
        }
    }
    ...
    document.getElementById('valBalance').innerText = `Rp ${d.balance}`;
    document.getElementById('balCash').innerText = `Rp ${d.cash_balance}`;
    document.getElementById('balWallet').innerText = `Rp ${d.ewallet_balance}`;

    const miniList = document.getElementById('miniList');
    if(miniList) miniList.innerHTML = d.recents.slice(0,5).map(r => createRow(r)).join('') || '<div class="flex flex-col items-center justify-center py-6 text-slate-500 gap-2"><i class="fas fa-box-open text-2xl"></i><span class="text-xs">Belum ada data</span></div>';
    
    filterHistory('ALL');

    // UNIFIED PRO FEATURES LOGIC
    ...
    if(d.is_prem || d.is_vip || d.is_admin) {
        ...
        if(targetElem) {
            targetElem.innerText = `Rp ${userBudget.toLocaleString('id-ID')}`;
            usedElem.innerText = `Rp ${expenseTotal.toLocaleString('id-ID')}`;
            let pct = userBudget > 0 ? (expenseTotal / userBudget) * 100 : 0;
            pctElem.innerText = `${Math.round(pct)}%`;
            barElem.style.width = `${pct}%`;
            if(pct >= 100) { 
                barElem.className = "shadow-none flex flex-col text-center whitespace-nowrap text-white justify-center bg-rose-600 transition-all duration-500 animate-pulse";
                if(expenseTotal > userBudget && userBudget > 0) tg.showPopup({title:"⚠️ Peringatan", message:"Melebihi batas budget!"});
        ...
        if(d.chart_values) {
            ...
            document.getElementById('topCatAmount').innerText = `Rp ${parseInt(maxVal).toLocaleString('id-ID')}`;

            let monthlyExp = 0;
            if(d.monthly_exp && d.monthly_exp.length > 0) monthlyExp = d.monthly_exp[d.monthly_exp.length - 1];
            const today = new Date().getDate();
            const avg = monthlyExp / (today || 1);
            document.getElementById('dailyAvg').innerText = `Rp ${parseInt(avg).toLocaleString('id-ID')}`;
        }
    ...
    if(ctx) {
        if(d.chart_values && d.chart_values.length > 0) {
            ...
            document.getElementById('chartLegend').innerHTML = d.chart_labels.map((l,i)=>`<div class="flex justify-between items-center mb-1"><div class="flex items-center gap-2"><span class="w-2 h-2 rounded-full" style="background-color:${['#a855f7','#3b82f6','#ef4444','#f59e0b','#10b981','#6366f1','#ec4899'][i%7]}"></span><span class="text-slate-300 text-xs">${l}</span></div><span class="font-mono text-slate-400 text-xs font-bold">${parseInt(d.chart_values[i]).toLocaleString('id-ID')}</span></div>`).join('');
...
window.setManualType = function(type) {
    ...
    if(type === 'OUT') {
        ...
        // Note: We handle the value logic in submitManual to avoid select box issues
    }
}

window.submitManual = function() {
    const amt = parseInt(document.getElementById('manualAmount').value); 
    const desc = document.getElementById('manualDesc').value; 
    let cat = document.getElementById('manualCat').value; 
    const wal = document.getElementById('manualWallet').value;
    
    if(!amt || !desc) return alert("Lengkapi data!");
    
    // LOGIC PERBAIKAN: Jika Pemasukan, paksa kategori jadi 'Pemasukan'
    if (currentManualType === 'IN') {
        cat = 'Pemasukan';
    } else {
        if ((wal === 'Cash' && amt > currentCashBal) || (wal === 'E-Wallet' && amt > currentWalletBal)) { 
            showError(`Saldo ${wal} tidak cukup!`); return; 
        }
    }
    ...
    fd.append('wallet', wal); 
    fd.append('type', currentManualType); 
    fd.append('mode', 'manual');
    
    const btn = document.querySelector('#page-manual button[onclick="submitManual()"]'); 
    ...
    fetch('/add_transaction', {method:'POST', body:fd}).then(r=>r.json()).then(d=>{
        ...
        if(d.status==='success') { 
            ...
            tg.showPopup({title:"Sukses", message:"Tersimpan!"}); 
        } 
        else showError(d.message);
    });
}
    ...
    if(mediaRecorder && mediaRecorder.state === "recording") { mediaRecorder.stop(); if(recInd) recInd.classList.add('hidden'); micBtn.classList.remove('bg-rose-500/10', 'border-rose-500'); } 
        ...
        navigator.mediaDevices.getUserMedia({ audio: true }).then(stream => {
            ...
            mediaRecorder.onstop = () => {
                ...
                const fd = new FormData(); fd.append('media_file', blob, 'voice.ogg'); fd.append('mode', 'voice');
...
function createRow(r) {
    ...
    return `<div onclick="openEditModal(${r.id})" class="cursor-pointer bg-slate-800 p-3 rounded-xl border border-slate-700/50 flex items-center gap-3 active:scale-95 transition hover:bg-slate-700"><div class="w-10 h-10 rounded-full bg-slate-900 flex items-center justify-center shrink-0 border border-slate-700"><i class="fas ${icon} text-sm ${color}"></i></div><div class="flex-1 min-w-0"><p class="font-bold text-sm truncate text-white">${r.description}</p><div class="flex items-center gap-2 text-[10px] text-slate-400 mt-1"><span class="bg-slate-900 px-1.5 py-0.5 rounded border border-slate-700">${r.category}</span><span>•</span><span>${r.wallet}</span></div></div><p class="font-bold text-sm ${color} whitespace-nowrap">${sign} ${parseInt(r.amount).toLocaleString('id-ID')}</p></div>`;
}

function filterHistory(type) {
    document.querySelectorAll('.filter-btn').forEach(b => { b.className = "filter-btn px-4 py-2 border border-slate-700 rounded-full text-xs font-bold whitespace-nowrap bg-slate-800 text-slate-400 transition hover:bg-slate-700 normal-case"; const icon = b.querySelector('i'); if(icon) icon.className = icon.className.replace('text-white', ''); });
    if(event && event.target) { let target = event.target.closest('button'); if(target) target.className = "filter-btn active px-4 py-2 bg-purple-600 text-white rounded-full text-xs font-bold whitespace-nowrap transition shadow-md normal-case"; }
...
window.switchTab = function(id) {
    ...
    const header = document.getElementById('headerSubtitle'); if(header) { const titles = {'beranda': 'Dashboard', 'visual': 'Visualisasi', 'analisis': 'Analisis', 'riwayat': 'Riwayat', 'lainnya': 'Menu Lainnya', 'goals': 'Rencana'}; header.innerText = titles[id] || "Menu"; }
...
async function loadGoals() {
    ...
    if(d.status === 'success' && d.data.length > 0) {
        list.innerHTML = d.data.map(g => {
            ...
            // Format angka
            const currentFmt = parseInt(g.current).toLocaleString('id-ID');
            const targetFmt = parseInt(g.target).toLocaleString('id-ID');
            
            return `
            <div class="bg-slate-800 p-4 rounded-xl border border-slate-700 flex flex-col gap-3 group relative overflow-hidden">
                <div class="flex justify-between items-start z-10">
                    <div>
                        <span class="text-[10px] font-bold px-2 py-0.5 rounded ${pColor} mb-1 inline-block">${g.priority}</span>
                        <h4 class="font-bold text-sm text-white">${g.title}</h4>
                    </div>
                    <div class="flex gap-2">
                        <!-- TOMBOL ISI TABUNGAN (BARU) -->
                        <button onclick="openGoalDeposit(${g.id}, '${g.title}')" class="w-8 h-8 rounded-lg bg-emerald-500/10 text-emerald-500 flex items-center justify-center hover:bg-emerald-500 hover:text-white transition shadow-sm">
                            <i class="fas fa-plus"></i>
                        </button>
                        <button onclick="deleteGoal(${g.id})" class="w-8 h-8 rounded-lg bg-slate-700 text-slate-400 flex items-center justify-center hover:bg-rose-500 hover:text-white transition">
                            <i class="fas fa-trash"></i>
                        </button>
                    </div>
                </div>
                
                <div class="w-full bg-slate-900 h-2 rounded-full overflow-hidden z-10 border border-slate-700/50">
                    <div class="bg-gradient-to-r from-purple-600 to-indigo-500 h-full transition-all duration-1000" style="width:${pct}%"></div>
                </div>
                
                <div class="flex justify-between text-[10px] text-slate-400 z-10 font-medium">
                    <span>Rp ${currentFmt} / ${targetFmt}</span>
                    <span class="${pct >= 100 ? 'text-emerald-400 font-bold' : ''}">${pct}%</span>
                </div>
                
                <!-- Background decoration -->
                <div class="absolute -bottom-6 -right-6 w-24 h-24 bg-purple-500/5 rounded-full blur-2xl z-0 group-hover:bg-purple-500/10 transition"></div>
            </div>`;
        }).join('');
    } else {
        list.innerHTML = '<div class="text-center py-10 text-slate-500 text-xs flex flex-col items-center gap-2"><i class="fas fa-bullseye text-2xl opacity-20"></i><span>Belum ada tujuan</span></div>';
    }
}

// === FITUR BARU: MANUAL DEPOSIT ===
window.openGoalDeposit = function(id, title) {
    document.getElementById('depGoalId').value = id;
    document.getElementById('goalDepositTitle').innerText = `Target: ${title}`;
    document.getElementById('depAmount').value = '';
    document.getElementById('goalDepositModal').classList.remove('hidden');
}

window.submitGoalDeposit = function() {
    const id = document.getElementById('depGoalId').value;
    ...
    // UI Feedback
    const btn = document.querySelector('#goalDepositModal button[onclick="submitGoalDeposit()"]');
    ...
    fetch('/api/goal_deposit', {method:'POST', body:fd})
    .then(r => r.json())
    .then(d => {
        ...
        if(d.status === 'success') {
            document.getElementById('goalDepositModal').classList.add('hidden');
            loadGoals(); // Refresh goals UI
            loadDashboardData(); // Refresh wallet balance UI
            tg.showPopup({title:"Berhasil", message:"Saldo berhasil ditambahkan!"});
            tg.HapticFeedback.notificationOccurred('success');
        } else {
            showError(d.message);
        }
...
// === UPDATE: ACTIONABLE AI OPTIMIZE ===
window.optimizeGoals = function() {
    const btnOpt = document.querySelector('button[onclick="optimizeGoals()"]');
    const oldIcon = btnOpt.innerHTML;
    btnOpt.innerHTML = '<i class="fas fa-circle-notch fa-spin"></i> Analyzing...';
    btnOpt.disabled = true;

    fetch('/api/optimize_goals').then(r=>r.json()).then(d=>{
        btnOpt.innerHTML = oldIcon;
        ...
        if(d.status==='success') {
            ...
            if(d.action) {
                // Format amount
                const amtFmt = parseInt(d.action.amount).toLocaleString('id-ID');
                
                actions.innerHTML = `
                <button onclick="executeAiAction(${d.action.goal_id}, '${d.action.wallet}', ${d.action.amount})" 
                    class="w-full bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 text-white py-3.5 rounded-xl text-sm font-bold shadow-lg shadow-emerald-900/20 active:scale-95 transition flex items-center justify-center gap-2 animate-pulse">
                    <i class="fas fa-coins"></i> Tabung Rp ${amtFmt} ke ${d.action.goal_title}
                </button>
                <button onclick="closeResultModal()" class="w-full text-slate-400 py-2 text-xs hover:text-white transition">
                    Nanti Saja
                </button>
                `;
            } else {
                // Default Close Button
                actions.innerHTML = `<button onclick="closeResultModal()" class="w-full bg-slate-700 hover:bg-slate-600 text-white py-3 rounded-xl font-bold transition">Tutup</button>`;
            }
        ...
        btnOpt.innerHTML = oldIcon;
        btnOpt.disabled = false;
        showError("Gagal menghubungi AI");
    });
}

// Executing the AI Action
window.executeAiAction = function(goalId, wallet, amount) {
    // Re-use the existing deposit logic but automatic
    const fd = new FormData();
    fd.append('goal_id', goalId);
    fd.append('wallet_source', wallet);
    fd.append('amount', amount);
    
    // UI Feedback on the AI Modal
    const btn = document.querySelector('#resultActions button');
    ...
    .then(d => {
        if(d.status === 'success') {
            ...
            loadGoals();
            loadDashboardData();
            tg.showPopup({title:"Hebat!", message:`Berhasil menabung Rp ${parseInt(amount).toLocaleString('id-ID')}!`});
            tg.HapticFeedback.notificationOccurred('success');
        } else {
            closeResultModal();
            showError(d.message);
...
window.closeResultModal = function() {
    const m = document.getElementById('resultModal');
    m.classList.add('opacity-0'); m.querySelector('div').classList.remove('scale-100'); m.querySelector('div').classList.add('scale-90');
...
window.submitTransfer = function() {
    ...
    fetch('/api/transfer_balance', {method:'POST', body:fd}).then(r=>r.json()).then(d=>{
        btn.innerText=old; btn.disabled=false;
        if(d.status==='success') { document.getElementById('transferModal').classList.add('hidden'); loadDashboardData(); tg.showPopup({title:"Berhasil", message:"Transfer sukses!"}); } else alert(d.message);
    });
}
...
window.closeEditModal = function() {
    const m = document.getElementById('editModal'); const c = document.getElementById('editModalContent'); if(m) { m.classList.add('opacity-0'); if(c) { c.classList.remove('scale-100'); c.classList.add('scale-95'); } setTimeout(() => m.classList.add('hidden'), 300); }
}
...
window.downloadExcel = function() {
    ...
    fetch('/api/download_excel').then(async r => {
        if (!r.ok) throw new Error((await r.json()).message || "Gagal download");
        return r.blob();
    }).then(blob=>{
        loader.style.display='none';
        if(navigator.share) {
            const file = new File([blob], "Laporan.xlsx", {type: blob.type});
            navigator.share({files:[file], title:"Laporan Keuangan"}).catch(e=>console.log(e));
        } else {
            const url = window.URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = `Laporan.xlsx`; document.body.appendChild(a); a.click(); a.remove();
        }
